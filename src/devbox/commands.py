...
from devbox.cloud_init import load_template
...
def write_ssh_config(
    ...
    Raises:
        ValueError: If instance has no IP address or name
    ...
    # Render SSH config from template
    config_content = load_template(
        {
...
def print_resource_header(console: Console, name: str) -> None:
...
def render_api_error(console: Console, error: Exception) -> None:
...
class UpCommand(BaseCommand):
    ...
    async def run(self) -> None:
        ...
        cloud_init_yaml = load_template(context, "cloud-init.yaml.j2")
