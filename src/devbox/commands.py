            ...
            match self.config.resource:
                ...
                case ListResource.instance_types:
                    ...
                    for name, item in sorted_items:
                        ...
                        # Format regions
                        if item.regions_with_capacity_available:
                            regions_list = [r.name.value for r in item.regions_with_capacity_available]
                            regions_display = f"[green]✓[/green] {', '.join(regions_list)}"
                        else:
                        ...
                        table.add_row("GPUs:", str(specs.gpus) if specs.gpus > 0 else "0 (CPU only)")
                        table.add_row("vCPUs:", str(specs.vcpus))
                        table.add_row("RAM:", f"{specs.memory_gib} GiB")
                        table.add_row("Storage:", f"{specs.storage_gib} GiB")
                        table.add_row("Price:", f"${price_dollars:.2f}/hour (${price_dollars * 24:.2f}/day, ${price_dollars * 730:.2f}/month)")
                    ...
                    # Show summary
                    available_count = sum(
                        1 for item in data.root.values()
                        if item.regions_with_capacity_available
                    )
                    self.console.print(
                ...
                case ListResource.images:
                    ...
                    if self.config.available_only:
                        images = [img for img in images if img.region.name.value in available_regions]
                    ...
                    by_image_group = defaultdict(list)
                    for img in images:
                        key = (img.family, img.description, img.version, img.architecture.value)
                        by_image_group[key].append(img)

                    # Sort groups
                    sorted_groups = sorted(by_image_group.keys())

                    # Display each image group
                    for family, description, version, architecture in sorted_groups:
                        group_images = by_image_group[(family, description, version, architecture)]

                        # Filter regional variants by available regions if requested
                        if self.config.available_only and available_regions:
                            group_images = [img for img in group_images if img.region.name.value in available_regions]
                    ...
                    # Show summary
                    total_variants = sum(len(group) for group in by_image_group.values())
                    if self.config.available_only and available_regions:
                        self.console.print(
                ...
                case ListResource.filesystems:
                    ...
                    if self.config.available_only and available_regions:
                        filesystems = [fs for fs in filesystems if fs.region.name.value in available_regions]
                    ...
                    for fs in filesystems:
                        ...
                        in_use_display = "[green]In use[/green]" if fs.is_in_use else "[dim]Not in use[/dim]"
                        ...
                        table.add_row("Created:", fs.created.strftime("%Y-%m-%d %H:%M:%S"))
                ...
                case ListResource.ssh_keys:
                    ...
                    for key in keys:
                        ...
                        self.console.print(f"\n[bold cyan]{key.name}[/bold cyan]")
                        table.add_row("ID:", key.id)
                        table.add_row("Public Key:", key.public_key)

                        self.console.print(table)

                    # Show summary
                    self.console.print(f"\n[bold]{len(keys)}[/bold] SSH keys")
                ...
                case ListResource.firewall_rulesets:
                    ...
                    if self.config.available_only and available_regions:
                        rulesets = [rs for rs in rulesets if rs.region.name.value in available_regions]
                    ...
                    for ruleset in rulesets:
                        ...
                        table.add_row("Status:", in_use_display)
                        table.add_row("Created:", ruleset.created.strftime("%Y-%m-%d %H:%M:%S"))
                        ...
                        if ruleset.rules:
                            ...
                            for rule in ruleset.rules:
                                # Format port range
                                if rule.port_range:
                                    if rule.port_range[0].root == rule.port_range[1].root:
...
class UpCommand(BaseCommand):
    ...
    async def run(self) -> None:
        """Execute up command."""
        # Load cloud-init
        cloud_init_yaml = load_cloud_init_template(
            self.config.cloud_init.template_path,
            self.config.cloud_init.use_default,
        )
        ...
        async with LambdaCloudClient(
            ...
            with self.console.status("[bold green]Launching instance..."):
            ...
            self.console.print(f"[green]✓[/green] Launched: [cyan]{instance_id}[/cyan]")

            # Wait if requested
            if self.config.wait_after_launch:
                inst = await self._wait_for_instance(client, instance_id)
                self.console.print(
